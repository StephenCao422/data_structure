...
class KDTree
{
  ...
  public:
    ...
    //KDTreeNode* buildTree(typename vector<Point<Dim>>::iterator start,typename vector<Point<Dim>>::iterator end,int depth);
    /**
    ...
    KDTreeNode* copyhelper(const KDTreeNode* subRoot);
    ...
    KDTreeNode* buildTree(vector<Point<Dim>>& newPoints,int start, int end, int depth);
    Point<Dim> nearesthelper(KDTreeNode* node, const Point<Dim>& query, int dimension) const;
  ...
  private:
     ...
     */
    //KDTreeNode * buildTree(const std::vector<Point<Dim>>& points, int left, int right, int depth);
};

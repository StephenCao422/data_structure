...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    size_t i = insertion_idx(subroot->elements, pair);

    /* TODO Your code goes here! */
    if (i < subroot->elements.size() && subroot->elements[i] == pair) return;
    if (subroot->is_leaf) subroot->elements.insert(subroot->elements.begin() + i, pair);
    else {
      insert(subroot->children[i], pair);
      if (subroot->children[i]->elements.size() > order-1) {

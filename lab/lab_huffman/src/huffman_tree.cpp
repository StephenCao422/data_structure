                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    if(single==nullptr && merge!=nullptr){
        mergeQueue.pop();
        return merge;
    }   else if (single!=nullptr && merge==nullptr){
        singleQueue.pop();
        return single;
    }   else if (single==nullptr && merge==nullptr){
        return nullptr;
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    while(!(singleQueue.empty() && (mergeQueue.size()==1))){
        ...
        //no sum function in frequency class
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    bool isleaf = bfile.getNextBit();

    if (!bfile.hasBits()) return NULL;
    else{
        if(isleaf){
            char element = bfile.getNextByte();
            node = new TreeNode(Frequency(element,0));
            return node;
        }
        else{
            node = new TreeNode(0);
